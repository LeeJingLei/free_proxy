...
按 `t` 进入网络测试页，再选择 `1` 网页连通性、`2` 代理建连延迟或 `3` 下载测速。连通性会完成一次真实 SOCKS5 CONNECT；延迟不包含本地 DNS，对每个目标完成 3 次“连接代理、SOCKS5 握手、目标 CONNECT”，显示中位数。测速最多下载 10 MiB，并实时显示 Mbps。测试过程中按 `q` 可取消，菜单中按 `q` 返回主界面。
