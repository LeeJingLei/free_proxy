...
按 `t` 进入网络测试页，再选择 `1` 网页连通性、`2` 端到端响应延迟或 `3` 下载测速。连通性会经 `free_proxy` 新建连接，完成一次真实 HTTP(S) 请求并确认收到有效 HTTP 响应。延迟对每个目标执行 3 次同样的请求，显示从发起请求到收到首个 HTTP 响应字节的中位数；其中包含 DNS、TCP、透明转发、SOCKS5、TLS（HTTPS）和服务器响应时间。连通性和响应延迟每完成一个目标都会立即显示，全部完成后再显示汇总结果。测速最多下载 10 MiB，并实时显示 Mbps。测试过程中按 `q` 可取消，菜单中按 `q` 返回主界面。
